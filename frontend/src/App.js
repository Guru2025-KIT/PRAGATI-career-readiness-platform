...
import AIAvatarInterviewer from './pages/AIAvatarInterviewer';
import WakeWordListener from './components/WakeWordListener';

// ── Keep ML service warm — pings Render every 10 min to prevent cold starts ──
(function keepMLWarm() {
  const ML_URL = process.env.REACT_APP_ML_URL || '';
  if (!ML_URL) return; // only runs if REACT_APP_ML_URL is set in .env
  function ping() { fetch(`${ML_URL}/health`, { method:'GET' }).catch(() => {}); }
  ping();
  ...
  return (
    <Routes>
      ...
      <Route path="/dashboard" element={<RequireAuth><DashboardLayout /></RequireAuth>}>
        ...
        <Route path="interview-prep/avatar" element={<AIAvatarInterviewer />} />
