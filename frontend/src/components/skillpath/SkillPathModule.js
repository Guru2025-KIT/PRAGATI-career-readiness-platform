...
const API =
  ...
  'https://pragati-backend-ixn3.onrender.com/api';
...
function AnalysisResults({ dbResult, full, onNewAnalysis }) {
  ...
  const missingSet = new Set(rawMissing.map(norm));
  const matched = rawMatched.filter(s => !missingSet.has(norm(s)));
...
function UploadForm({ user, onResult }) {
  ...
  // ── Refresh JWT before long ML call (prevents token expiry during 30-90s Render cold start) ──
  async function getFreshToken() {
    const token   = localStorage.getItem('pragati_token');
    const refresh = localStorage.getItem('pragati_refresh');
    ...
    } catch { /* network error — use existing token */ }
    return token;
  }

  // ── Fetch with 2-min timeout + escalating warm-up status messages ──────────
  async function fetchWithWarmup(url, options, onStatus) {
    const controller = new AbortController();
    const timeoutId  = setTimeout(() => controller.abort(), 120000); // 2 min hard limit

    const messages = [
      { delay:  5000, text: '⏳ AI engine is starting up...' },
      { delay: 20000, text: '🔄 ML service is waking up (may take 30–90s on first use)...' },
      { delay: 40000, text: '☕ Almost there — Render free tier takes a moment to wake...' },
      { delay: 70000, text: '🔃 Still loading — please hold on...' },
    ];
    const timers = messages.map(({ delay, text }) =>
      setTimeout(() => onStatus?.(text), delay)
    );
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        throw new Error('⏰ The AI service took too long to respond. Please try again — it should be much faster now that it has warmed up.');
      }
      throw err;
    }
  }
  ...
  async function handleAnalyze() {
    ...
    try {
      // ── Step 1: Refresh JWT BEFORE slow ML call ──
      ...
      // ── Step 3: Call ML with warm-up timeout + status messages ──
      const res = await fetchWithWarmup(
        `${API}/skillpath/analyze`,
        { method:'POST', headers:{ Authorization:`Bearer ${freshToken}` }, body:fd },
        (msg) => setError(msg)   // reuse error state for warm-up messages
      );

      const data = await res.json();
      if(!res.ok) throw new Error(data.error||'Analysis failed');
      setError(''); // clear any warm-up messages
      onResult(data);
  ...
  return (
    <div className="card" style={{ padding:'24px 24px',marginBottom:20,position:'relative' }}>
      ...
      {error && !['⏳','🔄','☕','🔃','⏰'].some(e => error.startsWith(e)) && (
        <div style={{ background:error.startsWith('⏰')?'#fef3c7':'#fee2e2', color:error.startsWith('⏰')?'#92400e':'#991b1b', padding:'9px 12px',borderRadius:8,fontSize:'.82rem',fontWeight:600,marginBottom:12 }}>{error}</div>
      )}
      {error && ['⏳','🔄','☕','🔃'].some(e => error.startsWith(e)) && (
        <div style={{ background:'rgba(83,22,151,0.06)',color:'#531697',padding:'9px 12px',borderRadius:8,fontSize:'.82rem',fontWeight:600,marginBottom:12,display:'flex',alignItems:'center',gap:8 }}>
          <span style={{ width:14,height:14,border:'2px solid rgba(83,22,151,0.3)',borderTopColor:'#531697',borderRadius:'50%',animation:'_sp .7s linear infinite',display:'inline-block',flexShrink:0 }}/>
          {error}
        </div>
      )}
      ...
      <button onClick={handleAnalyze} disabled={analyzing}
        ...
        {analyzing
        ? <><span style={{ width:18,height:18,border:'2.5px solid rgba(255,255,255,.3)',borderTopColor:'#fff',borderRadius:'50%',animation:'_sp .7s linear infinite',display:'inline-block' }}/>{error && error.startsWith('⏳') || error.startsWith('🔄') || error.startsWith('☕') || error.startsWith('🔃') ? error : 'Analyzing your resume…'}</>
        : '⚡ Analyze My Skills'}
      </button>
