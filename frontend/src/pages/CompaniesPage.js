            ...
            <Section title="🔁 Recruitment Rounds">
              {company.recruitmentRounds.map((r,i) => (
                <div key={i} style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:8 }}>
                  <span style={{ width:24, height:24, borderRadius:'50%', background:'rgba(83,22,151,.1)', color:'#531697', fontSize:'.72rem', fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{i+1}</span>
                  <span style={{ fontSize:'.82rem', color:'#3d4e6b', lineHeight:1.5 }}>{r}</span>
                </div>
              ))}
            </Section>
          )}
