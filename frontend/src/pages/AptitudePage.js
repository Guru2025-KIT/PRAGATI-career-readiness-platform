...
const ICONS = { 'Quantitative':'🔢','Logical Reasoning':'🧩','Verbal Ability':'📖','Technical':'💻','DSA Aptitude':'🌳','Data Interpretation':'📊' };
                ...
                return (
                  <div key={cat} className="card" style={{ padding:0, overflow:'hidden' }}>
                    ...
                    <button onClick={()=>setSelCat(isOpen?'':cat)}
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:'.92rem', color:'#0f1a2e' }}>{cat}</div>
      ...
      {stats.map(s=>(
        <div key={s.topic} style={{ marginBottom:8 }}>
          <div style={{ display:'flex', justifyContent:'space-between', fontSize:'.72rem', fontWeight:700, color:'#3d4e6b', marginBottom:3 }}>
            <span>{ICONS[s.topic]||''} {s.topic}</span>
            <span style={{ color:(s.accuracy*100)>=70?'#47d372':(s.accuracy*100)>=45?'#f59e0b':'#ef4444' }}>{Math.round(s.accuracy*100)}% ({s.correct}/{s.total})</span>
          </div>
          <div style={{ height:5, background:'#f0f3fa', borderRadius:999 }}>
            <div style={{ height:'100%', width:`${Math.round(s.accuracy*100)}%`, background:(s.accuracy*100)>=70?'#47d372':(s.accuracy*100)>=45?'#f59e0b':'#ef4444', borderRadius:999, transition:'width .9s' }} />
          </div>
        </div>
      ))}
    </div>
...
export default function AptitudePage() {
  ...
  async function handleStartPractice({topic,subtopic,shuffle}){
    const qs=await fetchQuestions({topic,subtopic,shuffle,count:20});
    if(!qs.length){alert('No questions found. Try selecting a different topic or seeding questions first.');return;}
