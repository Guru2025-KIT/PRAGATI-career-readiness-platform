...
function ActivityHeatmap({ submissions=[], currentStreak=0, maxStreak=0, compact=false }) {
  const counts = {};
  // Normalize to YYYY-MM-DD (handles ISO strings, timestamps, plain dates)
  (submissions || []).forEach(raw => {
    if (!raw) return;
    try {
      // ✅ Use LOCAL date (not UTC) so Indian timezone (UTC+5:30) doesn't shift date back by 1 day
      const d = new Date(raw);
      const norm = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
      counts[norm] = (counts[norm] || 0) + 1;
    } catch { counts[String(raw).slice(0, 10)] = (counts[String(raw).slice(0, 10)] || 0) + 1; }
  });
...
function StudentDash() {
  ...
  const load = useCallback(async () => {
    ...
    if (uid) {
      apiFetch(`/analytics/student-profile/${uid}`).then(d => {
  ...
  // ── Reactive heatmap: refresh when user solves a problem or completes quiz ──
  useEffect(() => {
    const handleActivity = () => {
      const uid = localStorage.getItem('pragati_uid') ||
                  JSON.parse(localStorage.getItem('pragati_user') || '{}')._id;
      if (!uid) return;
      const API2 = process.env.REACT_APP_API_URL || 'https://pragati-backend-ixn3.onrender.com/api';
      const token = localStorage.getItem('pragati_token');
      fetch(`${API2}/analytics/student-profile/${uid}`, {
        headers: { Authorization: `Bearer ${token}` }
      }).then(r => r.json()).then(d => { if (d) setMyActivityData(d); }).catch(() => {});
    };
    window.addEventListener('pragati:activity', handleActivity);
    const interval = setInterval(handleActivity, 90000); // refresh every 90s
    return () => {
      window.removeEventListener('pragati:activity', handleActivity);
  ...
  return (
    <div style={{ fontFamily:"'Nunito',sans-serif", maxWidth:1200, margin:'0 auto' }}>
      ...
      {/* ── KPI STATS ROW ────────────────────────────────────────────── */}
      <div className="dash-stats-grid" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:14 }}>
        {[
      ...
      <div className="dash-main-grid" style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:12, marginBottom:12 }}>
