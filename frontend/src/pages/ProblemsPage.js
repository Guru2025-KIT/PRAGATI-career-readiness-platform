...
export default function ProblemsPage() {
  ...
  useEffect(() => {
    ...
    // ── Auto-refresh at IST midnight so problem updates without manual reload ──
    function msUntilISTMidnight() {
      const now = new Date();
      const istOffset = 5.5 * 60 * 60 * 1000;
      const istNow    = new Date(now.getTime() + istOffset);
      const nextMid   = new Date(istNow);
      nextMid.setUTCHours(24, 0, 0, 0);  // next UTC midnight in IST context = next IST midnight
      return nextMid.getTime() - istNow.getTime() + 2000; // +2s buffer
    }
  ...
  async function handleSolve(problemId, notes, code, rating) {
    ...
    try {
      ...
      setMsg(`🎉 Solved! Streak: ${d.streak} days 🔥`);
      // Notify dashboard heatmap to refresh
      window.dispatchEvent(new CustomEvent('pragati:activity', { detail: { date: new Date().toISOString().slice(0,10) } }));
      fetchData();
