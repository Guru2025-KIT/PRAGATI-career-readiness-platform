...
const NAV_STUDENT = [
  ...
  { to:'/dashboard/interview-prep/avatar', icon:'🤖', label:'AI Interviewer' },
...
export default function DashboardLayout() {
  ...
  React.useEffect(() => {
    ...
    const base  = process.env.REACT_APP_API_URL || 'https://pragati-backend-ixn3.onrender.com/api';
      ...
      <div style={{ position:'fixed', inset:0, background:'rgba(4,44,93,0.6)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }} onClick={()=>setShowEditProfile(false)}>
        <div style={{ background:dropBg, borderRadius:20, padding:'24px 28px', maxWidth:560, width:'100%', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 80px rgba(4,44,93,0.3)' }} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{ 
            ...
            padding: '6px 10px', fontSize: 11, color: '#7a8ba8' 
          }}>
            <span style={{ 
  ...
  return (
    <div className="pragati-shell" style={{ background:pageBg, fontFamily:"'Nunito',sans-serif" }}>
      <style>{`
      ...
      {/* ── Main area ───────────────────────────────────────────────────── */}
      <div className="pragati-main" style={{ flex:1 }}>

        {/* Header — hidden on GD room so the call gets full screen */}
        {!isGDRoom && (
          <header className="pragati-header" style={{ background:headerBg, borderBottom:`1px solid ${headerBrd}`, display:'flex', alignItems:'center', padding:'0 24px', gap:12, position:'sticky', top:0, zIndex:100, boxShadow:'0 2px 8px rgba(4,44,93,0.05)', flexShrink:0 }}>
            ...
            {/* Streak */}
            {user?.role === 'student' && (
              <div className="streak-badge" style={{ display:'flex', alignItems:'center', gap:6, background:'linear-gradient(135deg,rgba(245,158,11,0.1),rgba(71,211,114,0.1))', padding:'5px 14px', borderRadius:999, border:'1px solid rgba(245,158,11,0.2)' }}>
        ...
        */}
        <main className="pragati-page" style={{
          flex: 1,
          ...(isGDRoom
          ...
          {isGDRoom
            ...
            : <div className="pragati-page-inner"><Outlet context={{ darkMode: dm }} /></div>
          }
        </main>

      </div>

      {/* ── Mobile bottom nav (shown only on ≤768px via CSS) ─────────────── */}
      {!isGDRoom && (
        <nav className="pragati-mobile-nav" data-dark={dm ? "true" : "false"}>
          {navItems.slice(0, 6).map(n => (
            <NavLink
              key={n.to}
              to={n.to}
              end={n.to === '/dashboard'}
              className={({ isActive }) => `pragati-mobile-nav-item${isActive ? ' active' : ''}`}
            >
              <span className="pragati-mobile-nav-icon">{n.icon}</span>
              <span className="pragati-mobile-nav-label">{n.label}</span>
            </NavLink>
          ))}
          <button
            className="pragati-mobile-nav-item"
            onClick={() => setDropOpen(o => !o)}
            style={{ border:'none', cursor:'pointer' }}
          >
            <span className="pragati-mobile-nav-icon">👤</span>
            <span className="pragati-mobile-nav-label">Profile</span>
          </button>
        </nav>
      )}
    </div>
