...
// IST midnight — Render runs UTC, Indian users need IST (UTC+5:30) day boundary
...
// Keep todayLocal as alias for backward compat with streak logic
function todayLocal() { return todayIST(); }

function isSameLocalDay(d1, d2) {
  if (!d1 || !d2) return false;
  // Compare in IST
  const istOffset = 5.5 * 60 * 60 * 1000;
  const a = new Date(new Date(d1).getTime() + istOffset);
  const b = new Date(new Date(d2).getTime() + istOffset);
