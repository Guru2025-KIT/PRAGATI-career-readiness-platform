...
// ─────────────────────────────────────────────────────────────────────────────
// SEED ROUTES — open in browser to populate MongoDB Atlas
// https://pragati-backend-ixn3.onrender.com/api/debug/seed-all   ← run this first
// ─────────────────────────────────────────────────────────────────────────────

// GET /api/debug/seed-demo  — creates 3 demo accounts
router.get('/seed-demo', async (req, res) => {
  try {
    const User = require('../models/User.model');
    await User.deleteMany({
      email: { $in: ['admin@pragati.edu', 'faculty@pragati.edu', 'student@pragati.edu'] }
    });
    const users = [
      { name: 'Platform Admin', email: 'admin@pragati.edu',   password: 'Admin@123',   role: 'admin',   department: 'CSAIML' },
      { name: 'Faculty Demo',   email: 'faculty@pragati.edu', password: 'Faculty@123', role: 'faculty', department: 'CSAIML' },
      { name: 'Student Demo',   email: 'student@pragati.edu', password: 'Student@123', role: 'student', department: 'CSAIML', year: 3, rollNumber: 'PRAGATI001', prn: 'PRN2024001', division: 'A' },
    ];
    for (const u of users) await new User(u).save();
    res.json({
      success: true,
      message: '3 demo users seeded',
      credentials: {
        admin:   { email: 'admin@pragati.edu',   password: 'Admin@123' },
        faculty: { email: 'faculty@pragati.edu', password: 'Faculty@123' },
        student: { email: 'student@pragati.edu', password: 'Student@123' },
      },
    });
  } catch (err) {
...
// GET /api/debug/seed-companies  — seeds all company profiles
router.get('/seed-companies', async (req, res) => {
  try {
    const seedFn = require('../utils/companySeed');
    await seedFn();
    res.json({ success: true, message: 'Companies seeded successfully' });
  } catch (err) {
    console.error('[seed-companies]', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// GET /api/debug/seed-aptitude  — seeds 900+ aptitude questions
router.get('/seed-aptitude', async (req, res) => {
  try {
    const seedFn = require('../utils/aptitude-seed-full');
    await seedFn();
    const { AptitudeQuestion } = require('../models');
    const total = await AptitudeQuestion.countDocuments();
    res.json({ success: true, message: `Aptitude questions seeded. Total in DB: ${total}` });
  } catch (err) {
    console.error('[seed-aptitude]', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// GET /api/debug/seed-problems  — seeds LeetCode problems
router.get('/seed-problems', async (req, res) => {
  try {
    const seedFn = require('../utils/leetcode-problems-seed');
    const result = await seedFn();
    res.json({ success: true, message: 'Problems seeded successfully', ...result });
  } catch (err) {
    console.error('[seed-problems]', err.message);
    res.status(500).json({ success: false, error: err.message });
  }
});

// GET /api/debug/seed-all  ⭐ MASTER — seeds everything in order
router.get('/seed-all', async (req, res) => {
  ...
  try {
    const User = require('../models/User.model');
    await User.deleteMany({ email: { $in: ['admin@pragati.edu', 'faculty@pragati.edu', 'student@pragati.edu'] } });
    const users = [
      { name: 'Platform Admin', email: 'admin@pragati.edu',   password: 'Admin@123',   role: 'admin',   department: 'CSAIML' },
      { name: 'Faculty Demo',   email: 'faculty@pragati.edu', password: 'Faculty@123', role: 'faculty', department: 'CSAIML' },
      { name: 'Student Demo',   email: 'student@pragati.edu', password: 'Student@123', role: 'student', department: 'CSAIML', year: 3, rollNumber: 'PRAGATI001', prn: 'PRN2024001', division: 'A' },
    ...
    for (const u of users) await new User(u).save();
    results.users = '✅ 3 demo users created';
  ...
  try {
    ...
    results.companies = `✅ Companies seeded (${count} total)`;
  } catch (e) { errors.companies = e.message; }

  // 3. Aptitude questions
  try {
    const seedAptitude = require('../utils/aptitude-seed-full');
    await seedAptitude();
    const { AptitudeQuestion } = require('../models');
    const count = await AptitudeQuestion.countDocuments();
    results.aptitude = `✅ Aptitude questions seeded (${count} total)`;
  } catch (e) { errors.aptitude = e.message; }

  // 4. LeetCode problems
  try {
    const seedProblems = require('../utils/leetcode-problems-seed');
    const r = await seedProblems();
    results.problems = `✅ Problems seeded (${r?.total || 0} total)`;
  } catch (e) { errors.problems = e.message; }

  const hasErrors = Object.keys(errors).length > 0;
  res.status(hasErrors ? 207 : 200).json({
    success: !hasErrors,
    message: hasErrors ? 'Seeding completed with some errors — check errors field' : '🎉 All data seeded successfully!',
    results,
...
// GET /api/debug/status  — check DB counts at a glance
router.get('/status', async (req, res) => {
  try {
    const { Company, AptitudeQuestion, Problem } = require('../models');
    const User = require('../models/User.model');
    const [users, companies, questions, problems] = await Promise.all([
      User.countDocuments(),
      Company.countDocuments(),
      AptitudeQuestion.countDocuments(),
      Problem.countDocuments(),
    ]);
    res.json({ users, companies, questions, problems });
  } catch (err) {
