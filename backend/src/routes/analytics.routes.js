...
router.get('/leaderboard', authenticate, async (req, res) => {
  try {
    const { AptitudeAttempt } = require('../models/index');
    const limit = Math.min(Number(req.query.limit) || 10, 50);
    const students = await User.find({ role:'student', isActive:true })
      .select('name department year skillLevel streak atsScore totalProblemsSolved linkedinUrl githubUrl portfolioUrl');

    const aptStats = await AptitudeAttempt.aggregate([
      { $group:{ _id:'$userId', total:{$sum:1}, correct:{$sum:{$cond:['$correct',1,0]}} } }
    ]);
    const aptMap = {};
    aptStats.forEach(a => { aptMap[a._id.toString()] = a; });

    const scored = students.map(s => {
      const apt = aptMap[s._id.toString()] || { total:0, correct:0 };
      const aptScore    = apt.total > 0 ? Math.round((apt.correct / apt.total) * 100) : 0;
      const codingScore = Math.min((s.totalProblemsSolved || 0) * 5, 100);
      const actScore    = Math.min((s.streak || 0) * 3 + (s.atsScore || 0) * 0.2, 100);
      const totalScore  = Math.round(aptScore * 0.4 + codingScore * 0.35 + actScore * 0.25);
      return {
        _id: s._id, name: s.name, department: s.department, year: s.year,
        skillLevel: s.skillLevel, aptScore,
        codingProblems: s.totalProblemsSolved || 0,
        streak: s.streak || 0, atsScore: s.atsScore || 0,
    ...
    res.json({ leaderboard: ranked, total: students.length });
  } catch(err) { res.status(500).json({ error:err.message }); }
});

// GET /api/analytics/student-profile/:id
router.get('/student-profile/:id', authenticate, authorize('faculty','admin'), async (req, res) => {
  try {
    const { AptitudeAttempt, UserProblem } = require('../models/index');
    ...
    const codingStats = await UserProblem.aggregate([
      { $match:{ userId: student._id } },
      { $lookup:{ from:'problems', localField:'problemId', foreignField:'_id', as:'p' } },
      { $unwind:{ path:'$p', preserveNullAndEmptyArrays:true } },
      { $group:{ _id:'$p.topic', total:{$sum:1}, solved:{$sum:{$cond:[{$eq:['$status','solved']},1,0]}} } },
      { $project:{ topic:'$_id', total:1, solved:1 } }
    ]);

    const recentActivity = await AptitudeAttempt.find({ userId: student._id })
    ...
    res.json({ student, aptStats, codingStats, recentActivity,
