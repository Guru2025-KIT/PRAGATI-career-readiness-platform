...
| Backend API | http://https://pragati-backend-ixn3.onrender.com |
| ML Service | http://process.env.ML_SERVICE_URL/docs |
...
### Test Backend health
```bash
curl http://https://pragati-backend-ixn3.onrender.com/health
# → {"status":"ok","service":"PRAGATI Backend"}
```

### Test login
```bash
curl -X POST http://https://pragati-backend-ixn3.onrender.com/api/auth/login \
...
```
http://process.env.ML_SERVICE_URL/docs
```
